@testset "Convolution methods" begin
    ...
    function conv_test(psf, img, img_out, dims, s)
        otf = fft(psf, dims)
        otf_r = rfft(psf, dims)
        # otf_p, conv_p = plan_conv(img, psf, dims, flags=FFTW.ESTIMATE)
    ...
    @testset "Check broadcasting convolution" begin
        ...
        psf = opt_cu(randn((5,6,7, 2, 3)), use_cuda)
   ...
   @testset "Check types" begin
        N = 10
        img = opt_cu(randn(Float32, (N, N)), use_cuda)
        psf = opt_cu(abs.(randn(Float32, (N, N))), use_cuda)
        dims = [1, 2] 
    ...
    @testset "Check type get_plan" begin
        @test plan_rfft === FourierTools.get_plan(typeof(1f0))  
        @test plan_fft === FourierTools.get_plan(typeof(1im))  
    end

    @testset "dims argument nothing" begin
        N = 5
        psf = opt_cu(abs.(randn((N, N, N))), use_cuda)
        img = opt_cu(randn((N, N, N)), use_cuda)
        dims = [1,2,3] 
