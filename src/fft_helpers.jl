...
However, the shift is done with `MutableShiftedArrays` and therefore doesn't allocate memory.
...
The shift is done with `MutableShiftedArrays` and therefore doesn't allocate memory.
